...
type ProjectContributorsProps = { people?: Contributor[], onSelect?: (contributorId: string) => void };
...
export default function ProjectContributors({
  ...
  return (
    <section className="h-full space-y-4 rounded-lg bg-white p-6 shadow">
      ...
      {people?.map((person) => (
        <Card key={person.id} onClick={() => onSelect?.(person.id)} className="bg-gray-200">
